...
##
##  Used to produce a random instance of Figure 5 from this paper
##
##  John Maidens
##  July 17, 2013
##
...
def generate_system(n):
    ...
    # Compute diagonal weightings of 1-norm in which f is contractive
        # c_tilde[n-1] is the contraction rate
    p_vec = []
    c_vec = []
    c_tilde = []
    q_vec = []
...
class ReachTrace:
    ...
    def algorithm3_bound(self, d0, M, c):
        self.d_norm1 = [d0]
        for i in range(len(self.T)-1):
            print exp(-c*(self.T[i+1]-self.T[i]))
            self.d_norm1.append(exp(-c*(self.T[i+1]-self.T[i]))*self.d_norm1[i])
...
class TraceArray(list):
    ...
    # Plot array of reach traces given by 2-norm data contained in trace.d1 and trace.d2 
    ...
    # Plot array of reach traces given by 1-norm data contained in trace.d_norm1 
...
# plot figures
num_plot = 100
Earray_Alg3.plotODE(num_plot, 'plot_cascade_ODE.pdf')
Earray_Alg3.plotReachSet_norm1(num_plot, 'plot_cascade_Reach.pdf')
